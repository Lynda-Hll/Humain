...
function Aceuil({}) {
  ...
  const getIcon = (icon) => {
    switch (icon) {
      case "SportsKabaddiOutlinedIcon":
        return <SportsKabaddiOutlinedIcon color="primary" fontSize="large" />;
      case "SentimentVerySatisfiedOutlinedIcon":
        return (
      ...
      case "PanToolOutlinedIcon":
        return <PanToolOutlinedIcon color="primary" fontSize="large" />;
      default:
        return null;
    }
  };
  ...
  useEffect(() => {
    axios.get("http://localhost:4000/api/users/lynda").then((response) => {
